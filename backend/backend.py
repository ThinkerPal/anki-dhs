...
def anki_specific_deck(deckid):
   ...
   while empty:
      ...
      cards = db.execute('SELECT CARDID, QUESTION, ANSWER,TIME,TOPIC FROM CARDS WHERE DECKID = ? AND FAMILIARITY = ?', (deckid, difficulty)).fetchall()
   ...
   card = {'cardid':card[0],'question':card[1], 'answer':card[2], 'familiarity':difficulty,'time':card[3],'topic':card[4]}
   return card
